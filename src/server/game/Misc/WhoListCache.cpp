...
void WhoListCacheMgr::Update()
{
	...
	for (HashMapHolder<Player>::MapType::const_iterator itr = m.begin(); itr != m.end(); ++itr)
	{
		...
		m_whoOpcodeList.push_back( WhoListPlayerInfo(itr->second->GetTeamId(), itr->second->GetSession()->GetSecurity(), itr->second->getLevel(), itr->second->getClass(), itr->second->getRace(), itr->second->GetZoneId(), itr->second->getGender(), wpname, wgname, aname, pname, gname) );
	}
}