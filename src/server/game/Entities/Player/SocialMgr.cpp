...
void SocialMgr::GetFriendInfo(Player* player, uint32 friendGUID, FriendInfo &friendInfo)
{
    ...
    if (pFriend && (!AccountMgr::IsPlayerAccount(security) || pFriend->GetTeamId() == teamId) && pFriend->IsVisibleGloballyFor(player))
    {
...
void SocialMgr::BroadcastToFriendListers(Player* player, WorldPacket* packet)
{
    ...
    for (SocialMap::const_iterator itr = m_socialMap.begin(); itr != m_socialMap.end(); ++itr)
    {
        ...
        if (itr2 != itr->second.m_playerSocialMap.end() && (itr2->second.Flags & SOCIAL_FLAG_FRIEND))
        {
            ...
            // PLAYER see his team only and PLAYER can't see MODERATOR, GAME MASTER, ADMINISTRATOR characters
            // MODERATOR, GAME MASTER, ADMINISTRATOR can see all
            if (pFriend && (!AccountMgr::IsPlayerAccount(pFriend->GetSession()->GetSecurity()) || pFriend->GetTeamId() == teamId) && player->IsVisibleGloballyFor(pFriend))
