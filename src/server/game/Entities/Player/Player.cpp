...
inline void KillRewarder::_InitXP(Player* player)
{
    ...
    if (!_killer->GetVehicle())
            _xp = Trinity::XP::Gain(player, _victim);

	if (_xp && _victim) // pussywizard: npcs with relatively low hp give lower exp
...
{ 
    ...
    if (m_deathTimer > 0 && !HasAuraType(SPELL_AURA_PREVENT_RESURRECTION))
    {
...
{ 
    ...
    uint32 bonus_xp = victim ? GetXPRestBonus(xp) : 0; // XP resting bonus
...
{ 
    ...
    data << uint32(0xC77) << uint32(1);
                                                            // 8 Arena season id
    data << uint32(0xF3D) << uint32(5);
...
{ 
	...
	bool rewarded = IsQuestRewarded(quest_id);
...
{ 
    ...
    for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
    {
        ...
        QuestStatusData& q_status = m_QuestStatus[questid];
        if (q_status.Status == QUEST_STATUS_INCOMPLETE && !GetGroup())
        {
...
{ 
    ...
    else
    {
        ...
        if (!IsBeingTeleported())
        {
            stmt->setUInt16(index++, (uint16)GetMapId());
            stmt->setUInt32(index++, (uint32)GetInstanceId());
            stmt->setUInt8(index++, (uint8(0) | uint8(0) << 4));
            stmt->setFloat(index++, finiteAlways(GetPositionX()));
            stmt->setFloat(index++, finiteAlways(GetPositionY()));
            stmt->setFloat(index++, finiteAlways(GetPositionZ()));
            stmt->setFloat(index++, finiteAlways(GetOrientation()));
