...
{
    public:
        CreatureTextBuilder(WorldObject* obj, ChatMsg msgtype, uint8 textGroup, uint32 id, WorldObject const* target)
            : _source(obj), _msgType(msgtype), _textGroup(textGroup), _textId(id), _target(target)
        {
        }

        size_t operator()(WorldPacket* data) const
        {
...
uint32 CreatureTextMgr::SendChat(Creature* source, uint8 textGroup, WorldObject const* whisperTarget /*= NULL*/, ChatMsg msgType /*= CHAT_MSG_ADDON*/, Language language /*= LANG_ADDON*/, CreatureTextRange range /*= TEXT_RANGE_NORMAL*/, uint32 sound /*= 0*/, TeamId teamId /*= TEAM_NEUTRAL*/, bool gmOnly /*= false*/, Player* srcPlr /*= NULL*/)
{
    ...
    {
        CreatureTextBuilder builder(finalSource, finalType, iter->group, iter->id, whisperTarget);
        SendChatPacket(finalSource, builder, finalType, whisperTarget, range, teamId, gmOnly);
    }
...
std::string CreatureTextMgr::GetChatString(uint32 entry, uint8 textGroup, uint32 id) const
{
